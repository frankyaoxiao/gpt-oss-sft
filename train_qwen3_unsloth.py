#!/usr/bin/env python3
"""
Unsloth LoRA training script for Qwen3-235B-A22B.
...
Note: Qwen3 is a MoE model. Router layer is disabled by default during fine-tuning.
"""
...
def format_example(example, tokenizer):
    ...
    Handles 'messages' format (conversations) and 'text' format (documents).

    For Qwen3 with v2 data: <think> tags are already in the content field.
    Just apply the chat template directly.
    """
    if "messages" in example and example["messages"]:
        # Multi-turn conversation format
        # v2 data already has <think> tags in content - just pass through
...
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune Qwen3-235B-A22B with Unsloth LoRA")
    parser.add_argument(
        "--model_name_or_path",
        type=str,
        default="unsloth/Qwen3-235B-A22B-Instruct-2507",
        help="Model name or path (use unsloth/ prefix for optimized version)",
    )
    parser.add_argument(
        "--dataset_path",
        type=str,
        default="/mnt/polished-lake/home/fxiao-two/OLMo-core/data/merged_sft_32b_v2.jsonl",
        help="Path to the converted dataset",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/mnt/polished-lake/home/fxiao-two/gptoss_ft/output_qwen3",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="LoRA dropout (0 recommended for MoE models)",
    )
...
def main():
    ...
    print("=" * 60)
    print("Qwen3-235B-A22B Unsloth LoRA Training")
    print("=" * 60)
    ...
    # Note: For MoE models, Unsloth handles the router layer automatically
    ...
    # Add LoRA adapters
    # Note: For MoE models, Unsloth disables router layer fine-tuning by default
